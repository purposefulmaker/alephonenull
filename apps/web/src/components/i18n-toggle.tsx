      ...
      >
        <div className="w-full">
          {locales.map(([locale, label]) => (
            <DropdownMenuItem
              key={locale}
              onClick={() => locale !== 'pt' ? changeLocale(locale as LocaleOptions) : undefined}
              disabled={currentLocale === locale || locale === 'pt'}
              className={locale === 'pt' ? 'opacity-50 cursor-not-allowed' : ''}
            >
              {label}
            </DropdownMenuItem>
          ))}
