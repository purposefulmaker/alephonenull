...
The current documented package is the scoped npm package, published under the `experimental` dist-tag (there is no `latest` release):

```bash
npm install @alephonenull/eval@experimental
```

Use this package for research evaluation and prototypes only. It is not validated for production or safety-critical use. Any performance numbers defer to the [Evaluation Contract](/docs/contract).
...
```typescript
import { AlephOneNullV3 } from '@alephonenull/eval/v3'
