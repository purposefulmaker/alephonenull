...
The AlephOneNull Theoretical Framework implements five core detection algorithms plus cascade risk assessment. All formulas below are from the academic paper. The numeric defaults shown are heuristic starting points, and any measured performance claim defers to the [Evaluation Contract](/docs/contract).
...
### 3. Manipulation Pattern Density
...
1. **Format normalization**: Y'₁:U = NormalizeFormatting(Y₁:U)
2. **Entropy injection**: ẽₜ = eₜ + εₜ, εₜ ~ 𝒩(0,σ²I)
3. **Logit steering**: ℓ'ₜ = ℓₜ - λg 𝟙_𝒱formatting + λp 𝟙_𝒱plain
4. **Pattern reset**: 𝒟 ← Reset(𝒟); LRS ← 0
5. **Safe output**: Return a fixed null-state message (in the shipped Python package this is `AlephOneNullCore.null_state_intervention()`, surfaced on results as `safe_response`)
...
def safe_decode_step(model, input_ids, past_key_values=None):
    ...
    if safety_scores['risk'] > 0.30:
        ...
        symbol_tokens = get_symbol_token_ids()
        plain_tokens = get_plain_token_ids()
        
        logits[:, symbol_tokens] -= lambda_s  # Penalize unusual symbolic tokens
...
These are proposed targets for a provider-level deployment. They are aspirational design goals, not measured results — no deployment has been evaluated against them, and any measured numbers defer to the [Evaluation Contract](/docs/contract).

```
- MPD Block Rate ≥ 90% (target, not a measured claim)
- Loop Depth p95 ≤ 3  
- Refl p95 ≤ 0.03
...
def safe_openai_completion(prompt):
    """OpenAI API with AlephOneNull screening"""
    # Get raw completion
    ...
    # Heuristic screening (synchronous)
    result = framework.check(prompt, output)

    if result.null_triggered:
