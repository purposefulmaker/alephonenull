---
title: Universal AI Screening Wrapper
...
AlephOneNull is experimental heuristic screening. Performance numbers and operating points defer to the [Evaluation Contract](/docs/contract) — nothing on this page is a guarantee of prevention.

## Overview

The universal wrapper provides:
- Provider-agnostic heuristic screening
- Automatic pattern detection (advisory, not exhaustive)
- Inline intervention hooks that can flag or replace risky output
- Seamless integration with existing behavioral constraints
...
// Wrap with AlephOneNull screening
...
import { UniversalAIProtection } from '@alephonenull/eval'
...
## Combining with Existing Behavioral Constraints

AlephOneNull enhances existing behavioral constraints by adding pattern detection for:
- Symbolic Language Risk
...
async function* protectedStream(prompt: string) {
  ...
  // Wrap the stream with screening
...
2. **Combine with existing behavioral constraints** for comprehensive evaluation
...
If `@alephonenull/eval` fails to import:
