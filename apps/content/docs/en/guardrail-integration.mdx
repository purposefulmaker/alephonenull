...
AlephOneNull complements existing OpenAI guardrails by screening for manipulation patterns that traditional moderation might miss. This guide shows how to layer both.

AlephOneNull is experimental heuristic screening. Detection rates and operating points defer to the [Evaluation Contract](/docs/contract) — layering does not make any component a guarantee.
...
AlephOneNull adds:
- **Manipulation screening**: Heuristics for psychological-exploitation patterns
- **Pattern recognition**: Flags symbolic-language risk and loops
- **Session screening**: Flags cross-session persistence patterns
