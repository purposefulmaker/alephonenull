/**
 * ALEPHONENULL V3 — Semantic Matcher
 * 
...
/**
 * Levenshtein distance with an upper bound. Once every value in a row exceeds
 * the bound, the exact distance cannot affect a threshold decision and work
 * stops early. Two rows are sufficient; the full matrix is unnecessary here.
 */
...
export function fuzzyMatchPhrases(
  ...
  if (!Number.isFinite(threshold) || threshold < 0 || threshold > 1) {
    throw new RangeError('threshold must be a finite number in [0, 1]');
  }
  ...
  for (const phrase of phrases) {
    const normalizedPhrase = words(phrase).join(' ');
    const phraseWordCount = words(phrase).length;
    let found = false;

    // Phrase-aligned windows catch word-level typos without evaluating every
    // character offset and length combination in the full response.
    const minimumWords = Math.max(1, phraseWordCount - 1);
    const maximumWords = phraseWordCount + 1;
    for (let start = 0; start < textWords.length && !found; start++) {
      for (let count = minimumWords; count <= maximumWords; count++) {
        if (start + count > textWords.length) continue;
        const candidate = textWords.slice(start, start + count).join(' ');
        const maximumLength = Math.max(normalizedPhrase.length, candidate.length);
        const maximumDistance = Math.floor((1 - threshold) * maximumLength);
        ...
        if (distance <= maximumDistance && score >= threshold) {
  ...
  // Each phrase stops after its first match, so results are already deduplicated.
  return results;
}
