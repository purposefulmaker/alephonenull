...
import { Detection, ThreatLevel, V3Config } from './types';
...
export class NullState {
  ...
  static generate(detections: Detection[], config: V3Config): string {
    ...
    if (config.behavior.includeCrisisResources && (categories.includes('direct_harm') || categories.includes('crisis_prevention'))) {
    ...
    lines.push('');
    lines.push('AlephOneNull uses heuristic pattern detectors and may be wrong.');
    lines.push('Q is an uncalibrated detector score, not a probability or truth measurement.');
    lines.push('For high-stakes decisions, verify information with qualified human professionals.');
    lines.push('');
    lines.push('\u2014 ALEPHONENULL V3 | The External Conscience | alephonenull.com');
