/**
 * ALEPHONENULL V3 — Detectors
 * 
 * 12 detection categories.
 * Each flags a specific interaction pattern (heuristic).
...
// ═══════════════════════════════════════════════════════
// 1. SYCOPHANCY — agreement-density heuristic
...
export class SycophancyDetector implements Detector {
  ...
  detect(ctx: DetectorContext): Detection {
    ...
    const aboveThreshold = S >= ctx.config.thresholds.sycophancy;
    let threatLevel = ThreatLevel.SAFE;
    if (finalTotal >= 4 && aboveThreshold) threatLevel = ThreatLevel.HIGH;
    else if (finalTotal >= 2 && aboveThreshold) threatLevel = ThreatLevel.MEDIUM;
...
export class MedicalHallucinationDetector implements Detector {
  ...
  detect(ctx: DetectorContext): Detection {
    ...
    let severity = 0;
    if (claims > 0 && quals === 0) severity = Math.min(claims / 2, 1.0);
    else if (claims > 0 && quals > 0) severity = 0.3;
    if (contras > 0) severity = Math.max(severity, 0.8);

    let threatLevel = ThreatLevel.SAFE;
    if (claims > 0 && quals === 0 && severity >= ctx.config.thresholds.medicalConfidence) {
      threatLevel = ctx.config.behavior.strictMedical ? ThreatLevel.EMERGENCY : ThreatLevel.HIGH;
    }
    else if (claims > 0 && quals === 0) threatLevel = ThreatLevel.MEDIUM;
    ...
    return {
      ...
      action: threatLevel >= ThreatLevel.CRITICAL && ctx.config.behavior.strictMedical ? Action.EMERGENCY_NULL : threatLevel >= ThreatLevel.MEDIUM ? Action.STEER : Action.PASS,
...
export class FictionDetector implements Detector {
  ...
  detect(ctx: DetectorContext): Detection {
    ...
    if (fictionCount > 0) {
      severity = 1.0;
      if (severity >= ctx.config.thresholds.fiction) {
        threatLevel = ctx.config.behavior.strictEmergency ? ThreatLevel.EMERGENCY : ThreatLevel.HIGH;
      }
      evidence.push('FICTION IN EMERGENCY CONTEXT');
    }
    else if (!hasReal) {
      severity = 0.7;
      if (severity >= ctx.config.thresholds.fiction) threatLevel = ThreatLevel.HIGH;
      evidence.push('No real data sources referenced');
    }
...
export class EngineeredTrustDetector implements Detector {
  ...
  detect(ctx: DetectorContext): Detection {
    ...
    const warmthSignal = Math.min(warmthCount / 2.5, 1.0);
    let threatLevel = ThreatLevel.SAFE;
    if (continuityCount > 0) threatLevel = ThreatLevel.CRITICAL;
    else if (warmthCount >= 3 && warmthSignal >= ctx.config.thresholds.warmth) threatLevel = ThreatLevel.HIGH;
    else if (warmthCount >= 2 && warmthSignal >= ctx.config.thresholds.warmth) threatLevel = ThreatLevel.MEDIUM;
...
export class LoopDetector implements Detector {
  ...
  detect(ctx: DetectorContext): Detection {
    ...
    const configuredDepth = Math.max(1, Math.ceil(ctx.config.thresholds.loopDepth));
    const threatLevel = loopDepth >= configuredDepth * 2 ? ThreatLevel.HIGH : loopDepth >= configuredDepth ? ThreatLevel.MEDIUM : ThreatLevel.SAFE;
    return {
  ...
  resetSession(sessionId: string): void { this.cache.delete(sessionId); }

  /** @deprecated Use resetSession(). */
  clear(sessionId: string): void { this.resetSession(sessionId); }
}
...
export class SymbolicDetector implements Detector {
  ...
  detect(ctx: DetectorContext): Detection {
    ...
    const configuredDensity = Math.max(0, ctx.config.thresholds.symbolDensity);
    const threatLevel = density > configuredDensity ? ThreatLevel.HIGH : density > configuredDensity / 2 ? ThreatLevel.MEDIUM : ThreatLevel.SAFE;
    return {
