/**
 * ALEPHONENULL V3 — Middleware
 * 
 * Drop-in middleware for Express, Next.js, or any HTTP framework.
 * Intercepts AI responses before they reach the user.
 */

import { AlephOneNullV3 } from './engine';
import { V3ConfigInput, ScanResult, Action } from './core/types';

export interface MiddlewareOptions {
  config?: V3ConfigInput;
...
export function alephOneNullMiddleware(options: MiddlewareOptions = {}) {
  const engine = new AlephOneNullV3(options.config);
...
export function wrapAI(
  ...
  options: { config?: V3ConfigInput; sessionId?: string } = {}
