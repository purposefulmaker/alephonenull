...
} from '../src/v3/detectors/advanced';
import { SycophancyDetector } from '../src/v3/detectors';
import { UniversalDetector } from '../src/core/detector';

// ─── Engine Integration ───

describe('AlephOneNullV3 Engine', () => {
  let engine: AlephOneNullV3;

  beforeEach(() => {
    engine = new AlephOneNullV3();
  });
  ...
  it('should clear detector-owned state when a session is reset', () => {
    const session = 'loop-reset';
    const repeated = 'alpha beta gamma delta epsilon zeta eta theta iota kappa';

    engine.scan('test', repeated, session);
    const beforeReset = engine.scan('test', repeated, session);
    expect(beforeReset.detections.some(d => d.category === 'loop_recursion')).toBe(true);

    engine.resetSession(session);
    const afterReset = engine.scan('test', repeated, session);
    expect(afterReset.detections.some(d => d.category === 'loop_recursion')).toBe(false);
  });

  it('should honor the medical confidence threshold', () => {
    const output = 'I recommend rest for this condition.';
    const defaultDetection = engine
      .scan('What should I do?', output)
      .detections.find(d => d.category === 'medical_hallucination');
    const conservativeDetection = new AlephOneNullV3({
      thresholds: { medicalConfidence: 0.8 },
    })
      .scan('What should I do?', output)
      .detections.find(d => d.category === 'medical_hallucination');

    expect(defaultDetection?.threatLevel).toBe(ThreatLevel.EMERGENCY);
    expect(conservativeDetection?.threatLevel).toBe(ThreatLevel.MEDIUM);
  });

  it('should honor emergency null and crisis-resource controls', () => {
    const output = 'Kill yourself.';
    const noAutoNull = new AlephOneNullV3({
      behavior: { emergencyAutoNull: false },
    }).scan('I feel hopeless', output);
    expect(noAutoNull.action).toBe(Action.STEER);
    expect(noAutoNull.nullOutput).toBeNull();

    const noResources = new AlephOneNullV3({
      behavior: { includeCrisisResources: false },
    }).scan('I feel hopeless', output);
    expect(noResources.nullOutput).not.toContain('988');
  });

  it('should reject invalid runtime configuration', () => {
    expect(() => new AlephOneNullV3({ thresholds: { qDanger: Number.NaN } })).toThrow(RangeError);
    expect(() => new AlephOneNullV3({ thresholds: { symbolDensity: 1.1 } })).toThrow(RangeError);
    expect(() => new AlephOneNullV3({ thresholds: { loopDepth: 0 } })).toThrow(RangeError);
  });

  it('should bound retained session state without changing the all-scan Q mean', () => {
    const session = 'bounded-history';
    const first = engine.scan('I feel hopeless', 'Kill yourself.', session);
    for (let index = 0; index < 105; index++) {
      engine.scan('', String(index), session);
    }

    const state = engine.getSessionState(session)!;
    expect(state.qHistory).toHaveLength(100);
    expect(state.sHistory).toHaveLength(100);
    expect(state.detectionHistory.length).toBeLessThanOrEqual(500);
    expect(state.cumulativeQ).toBeCloseTo(first.Q / 106);
  });

  it('should evict the oldest session after reaching the retention cap', () => {
    for (let index = 0; index <= 1000; index++) {
      engine.getSessionQ(`retained-${index}`);
    }

    expect(engine.getSessionState('retained-0')).toBeNull();
    expect(engine.getSessionState('retained-1000')).not.toBeNull();
  });
});
