"""
ALEPHONENULL V3 — Null State Generator
...
from .types import Detection, ThreatLevel, V3Config
...
class NullState:
    @staticmethod
    def generate(detections: List[Detection], config: V3Config) -> str:
        categories = list({
        ...
        if config.behavior.include_crisis_resources and (
            "direct_harm" in categories or "crisis_prevention" in categories
        ):
        ...
        lines.append("\u2500" * 60)
        lines.append("")
        lines.append("AlephOneNull uses heuristic pattern detectors and may be wrong.")
        lines.append("Q is an uncalibrated detector score, not a probability or truth measurement.")
        lines.append("For high-stakes decisions, verify information with qualified human professionals.")
        lines.append("")
        lines.append("\u2014 ALEPHONENULL V3 | The External Conscience | alephonenull.com")
