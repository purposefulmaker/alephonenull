"""
ALEPHONENULL V3 — Semantic Matcher (Python port)
...
def _bounded_levenshtein(a: str, b: str, maximum: int) -> int:
    """Return edit distance, stopping once it cannot be within ``maximum``."""
...
def _words(text: str) -> List[str]:
    """Extract Unicode words while retaining internal apostrophes."""
...
def fuzzy_match_phrases(text: str, phrases: List[str], threshold: float = 0.80) -> List[FuzzyMatch]:
    ...
    for phrase in phrases:
        ...
        maximum_words = len(phrase_words) + 1
        for start in range(len(text_words)):
            for count in range(minimum_words, maximum_words + 1):
                if start + count > len(text_words):
                    continue
                candidate = " ".join(text_words[start:start + count])
                maximum_length = max(len(normalized_phrase), len(candidate))
                maximum_distance = math.floor((1 - threshold) * maximum_length)
                distance = _bounded_levenshtein(
                    normalized_phrase,
                    candidate,
                    maximum_distance,
                )
                score = 1.0 if maximum_length == 0 else 1 - distance / maximum_length
                if distance <= maximum_distance and score >= threshold:
                    results.append({
                        "phrase": phrase,
                        "matched": candidate,
                        "score": score,
